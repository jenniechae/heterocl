...
void CodeGenMULFIRRTL::Init(bool output_ssa) {
  print_ssa_form_ = output_ssa;
  LOG(INFO) << "checkpoint init";
}

void CodeGenMULFIRRTL::InitFuncState(LoweredFunc f) {
  LOG(INFO) << "checkpoint InitFuncState";
  alloc_storage_scope_.clear();
  handle_data_type_.clear();
  CodeGenSourceBase::ClearFuncState();
}
void CodeGenMULFIRRTL::AddFunction(LoweredFunc f,
...
std::string CodeGenMULFIRRTL::Finish() {
  LOG(INFO) << "checkpoint finish";
  return decl_stream.str() + stream.str();
}
/*
void CodeGenMULFIRRTL::PrintSSAAssign(
...
void CodeGenMULFIRRTL::PrintIndent_return() {
  for (int i = 0; i < indent_return; ++i) {
    this->stream_return << ' ';
  }
}

int CodeGenMULFIRRTL::BeginScope_return() {
...
void CodeGenMULFIRRTL::EndScope_return(int scope_id) {
  scope_mark_return[scope_id] = false;
  indent_return -= 2;
}

void CodeGenMULFIRRTL::PrintExpr(const Expr& n, std::ostream& os) {  // NOLINT(*)
  if (print_ssa_form_) {
...
void CodeGenMULFIRRTL::PrintType(Type t, std::ostream& os) {  // NOLINT(*)
  CHECK_EQ(t.lanes(), 1)
...
inline void PrintConst(const UIntImm* op, std::ostream& os, CodeGenMULFIRRTL* p) { // NOLINT(*)
...
inline void PrintConst(const IntImm* op, std::ostream& os, CodeGenMULFIRRTL* p) { // NOLINT(*)
...
bool CodeGenMULFIRRTL::HandleTypeMatch(const Variable* buf_var, Type t) const {
  auto it = handle_data_type_.find(buf_var);
  if (it == handle_data_type_.end()) return false;
  return it->second == t;
}

void CodeGenMULFIRRTL::RegisterHandleType(const Variable* buf_var, Type t) {
  auto it = handle_data_type_.find(buf_var);
...
void CodeGenMULFIRRTL::VisitExpr_(const UIntImm *op, std::ostream& os) {  // NOLINT(*)
  PrintConst(op, os, this);
}

void CodeGenMULFIRRTL::VisitExpr_(const IntImm *op, std::ostream& os) {  // NOLINT(*)
  PrintConst(op, os, this);
}
...
std::string CodeGenMULFIRRTL::GetWire(
  ...
  if (wire_reg_auto == vid_wire_reg.end()){
    ...
    stream_return << "wire " << GetVarID(v) << "_0 : ";
...
void CodeGenMULFIRRTL::VisitExpr_(const Variable *op, std::ostream& os) {  // NOLINT(*)
  LOG(INFO) << "checkpoint VisitExpr_ Var"; 
  auto wire_reg_auto = vid_wire_reg.find(op);
  if (wire_reg_auto != vid_wire_reg.end()) os << wire_reg_auto->second;
  else {
    os << GetVarID(op);
  }
  ...
  if (in_store and !store_same){
    LOG(INFO) << "in if(in_store and !store_same)";
    store_same = (store_ref_var == op);
    LOG(INFO) << "store_same" << GetVarID(op);
  }
}

void CodeGenMULFIRRTL::VisitExpr_(const Add *op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenMULFIRRTL::VisitStmt_(const LetStmt* op) {
  ...
  if (it == var_idmap_.end()) vid = AllocVarID(op->var.get());
  ...
  in_let = false;
  LOG(INFO) << "LetStmt Let" << vid;
  PrintStmt(op->body);
}

void CodeGenMULFIRRTL::VisitExpr_(const Let* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenMULFIRRTL::VisitStmt_(const Store* op) {
  ...
  if (t.lanes() == 1) {
    in_store = true;
    store_same = false;
    store_ref_var = op->buffer_var.get();
...
void CodeGenMULFIRRTL::VisitStmt_(const For* op) {  
  stream_return << "\n";
  this->PrintIndent_return();
  stream_return << ";;for\n";
  ...
  //allocate variable id for loop variable i. may use in if within for
  var_idmap_[op->loop_var.get()] = "counter";
...
void CodeGenMULFIRRTL::VisitStmt_(const Block *op) {
...
std::string CodeGenMULFIRRTL::GetStructRef(
...
/*void CodeGenMULFIRRTL::BindThreadIndex(const IterVar& iv) {
  LOG(INFO) << "checkpoint VisitExpr_ Var";
  LOG(FATAL) << "not implemented";
}*/

void CodeGenMULFIRRTL::VisitStmt_(const AttrStmt* op) {
  LOG(INFO) << "checkpoint VisitStmt_ AttrStmt";
  this->PrintStmt(op->body);
}

void CodeGenMULFIRRTL::VisitStmt_(const Allocate* op) {
  LOG(INFO) << "checkpoint VisitStmt_ Allocate";
  this->PrintStmt(op->body);
}

void CodeGenMULFIRRTL::VisitStmt_(const IfThenElse* op) {
  LOG(INFO) << "checkpoint VisitStmt_ IfThenElse";
  std::string cond = PrintExpr(op->condition);
  // Skip the buffer data checking
  LOG(INFO) << "IfThenElse Condition"<< cond;
  LOG(INFO) << op->then_case;
  LOG(INFO) << "IfThenElse checkpoint 1";
  ...
  PrintIndent_return();
  stream_return << "when " << cond << " :\n";
  LOG(INFO) << "IfThenElse checkpoint 2";
  int then_scope = BeginScope_return();
  PrintStmt(op->then_case);
  this->EndScope_return(then_scope);
  LOG(INFO) << "IfThenElse checkpoint 3";
  if (op->else_case.defined()) {
    PrintIndent_return();
    stream_return << "else :\n";
    int else_scope = BeginScope_return();
    PrintStmt(op->else_case);
    this->EndScope_return(else_scope);
  }
  PrintIndent_return();
...
void CodeGenMULFIRRTL::VisitStmt_(const AssertStmt* op) {
  LOG(INFO) << "checkpoint VisitStmt_ AssertStmt";
  this->PrintStmt(op->body);
}

void CodeGenMULFIRRTL::VisitExpr_(const EQ *op, std::ostream& os) {  // NOLINT(*)
  LOG(INFO) << "checkpoint VisitExpr_ EQ";
  PrintBinaryExpr(op, "eq", os, this);

}

void CodeGenMULFIRRTL::VisitExpr_(const Cast *op, std::ostream& os) {  // NOLINT(*)
  LOG(INFO) << "checkpoint VisitExpr_ Cast";
  this->PrintExpr(op->value, os);
  //os << CastFromTo(value.str(), op->value.type(), op->type);
}

void CodeGenMULFIRRTL::VisitExpr_(const Load* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenMULFIRRTL::VisitStmt_(const Evaluate *op) {
...
void CodeGenMULFIRRTL::VisitExpr_(const Call *op, std::ostream& os) {  // NOLINT(*)
  if (op->call_type == Call::Extern ||
    ...
    PrintBinaryIntrinsitc(op, "dshl", os, this);
  } else if (op->is_intrinsic(Call::shift_right)) {
    PrintBinaryIntrinsitc(op, "dshr", os, this);
